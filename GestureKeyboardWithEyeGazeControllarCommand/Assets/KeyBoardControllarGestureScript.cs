...
public class KeyBoardControllarGestureScript : MonoBehaviour
{
    ...
    private List<string> exapleSentencesList = ExampleSentences.ControllerGestureExampleSentencesForExperiment; // 例文のリスト
    ...
    {
        // 入力手法を記録
        inputDataList.input_method = "ControllarGesture";
    ...
    void Update()
    {
        ...
        if (isLeftHit || isRightHit)
        {
            ...
            // 候補ワードを選択した場合
            if (isRightHit && rightGameObj.CompareTag("CandidateWordLeft") && QuestContorollarAccessor.GetDownAButton() && guessedWordList.Length >= 1)  {
                checkTimerStart(); // 最初の入力文字であればタイマーをスタートする
                selectAudio.Play(); // 選択肢を選んだ時の音を出す
                AddWord(guessedWordList[0]);
                ResetGuessedTexts();
                ResetGuessedWordList();
            } else if(isLeftHit && leftGameObj.CompareTag("CandidateWordLeft") && QuestContorollarAccessor.GetDownXButton() && guessedWordList.Length >= 1) {
                checkTimerStart(); // 最初の入力文字であればタイマーをスタートする
                selectAudio.Play(); // 選択肢を選んだ時の音を出す
                AddWord(guessedWordList[0]);
                ResetGuessedTexts();
                ResetGuessedWordList();
            } else if (isRightHit && rightGameObj.CompareTag("CandidateWordMiddle") && QuestContorollarAccessor.GetDownAButton() && guessedWordList.Length >= 2)  {
                checkTimerStart(); // 最初の入力文字であればタイマーをスタートする
                selectAudio.Play(); // 選択肢を選んだ時の音を出す
                AddWord(guessedWordList[1]);
                ResetGuessedTexts();
                ResetGuessedWordList();
            } else if(isLeftHit && leftGameObj.CompareTag("CandidateWordMiddle") && QuestContorollarAccessor.GetDownXButton() && guessedWordList.Length >= 2) {
                checkTimerStart(); // 最初の入力文字であればタイマーをスタートする
                selectAudio.Play(); // 選択肢を選んだ時の音を出す
                AddWord(guessedWordList[1]);
                ResetGuessedTexts();
                ResetGuessedWordList();
            } else if (isRightHit && rightGameObj.CompareTag("CandidateWordRight") && QuestContorollarAccessor.GetDownAButton() && guessedWordList.Length >= 3)  {
                checkTimerStart(); // 最初の入力文字であればタイマーをスタートする
                selectAudio.Play(); // 選択肢を選んだ時の音を出す
                AddWord(guessedWordList[2]);
                ResetGuessedTexts();
                ResetGuessedWordList();
            } else if(isLeftHit && leftGameObj.CompareTag("CandidateWordRight") && QuestContorollarAccessor.GetDownXButton() && guessedWordList.Length >= 3) {
                checkTimerStart(); // 最初の入力文字であればタイマーをスタートする
                selectAudio.Play(); // 選択肢を選んだ時の音を出す
                AddWord(guessedWordList[2]);
                ResetGuessedTexts();
            ...
            // キーボードを選択した場合
            } else if ((isRightHit && rightGameObj.CompareTag("KeyBoard")) || (isLeftHit && leftGameObj.CompareTag("KeyBoard"))) {
                ...
                if (isRightInput) {
                    ...
                    // 入力モード時の処理
                    if (QuestContorollarAccessor.GetXButton()) {
                        checkTimerStart(); // 最初の入力文字であればタイマーをスタートする
                    ...
                    // 入力モードが終了したときの処理
                    if (QuestContorollarAccessor.GetUpXButton()) {
                        checkTimerStart(); // 最初の入力文字であればタイマーをスタートする
                    ...
                    // コントローラ＋ジェスチャーのGetDownは下に書く
                    // ジェスチャーの途中か否かで分ける必要がある

                    // 入力モードに入ったタイミングで既存の筆跡を消す
                    if (isRightHit && QuestContorollarAccessor.GetDownAButton())  {
                        checkTimerStart(); // 最初の入力文字であればタイマーをスタートする
                        ...
                        // ジェスチャーの始まりの場合
                            ...
                            if (rightKey != null) {
                                ...
                                int key_int = rightKey.keyCharacter.ToCharArray()[0];
                    ...
                    // 入力モードに入ったタイミングで既存の筆跡を消す
                    if (isLeftHit && QuestContorollarAccessor.GetDownXButton())  {
                        checkTimerStart(); // 最初の入力文字であればタイマーをスタートする
                        ...
                        // ジェスチャーの始まりの場合
                            ...
                            if (leftKey != null) {
                                int a_ind = (int)'a'; int z_ind = (int)'z';
                                int key_int = leftKey.keyCharacter.ToCharArray()[0];
                                if (a_ind <= key_int && key_int <= z_ind) {
