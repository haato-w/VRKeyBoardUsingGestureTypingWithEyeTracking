...
public class ControllerGesture : MonoBehaviour
{
    ...
    void Update() {
        ...
        if (isLeftHit || isRightHit)
        {
            ...
            // if (isLeftHit) { 
            //     Sphere.transform.position = leftHit.point;
            // } else {
            //     Sphere.transform.position = rightHit.point;
            // }
            ...
            if (gameObject.CompareTag("NextKey")) {
                ...
                if ((isRightHit && OVRInput.GetUp(OVRInput.Button.One)) || (isLeftHit && OVRInput.GetUp(OVRInput.Button.Three))) {
            ...
            // キーボードUIである場合
                ...
                if (isRightInput) {
                    ...
                    if (OVRInput.Get(OVRInput.Button.One)) {
                        ...
                        // ヒット座標を画像のピクセル座標に変換
                        Vector2 hitPosForPixel = HitPos2PixcelPos(hit.point.x, hit.point.y);
